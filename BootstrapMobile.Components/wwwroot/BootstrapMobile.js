...
var blazorGesture = function () {
    return {
        dotNetReference: null,

        init: function (dotNetReference, target, width) {
            this.dotNetReference = dotNetReference;
            let swiped = false;
            let startOffset = 0;
            const decelerationOnOverflow = 4;
            const revealWidth = width;
...
var toggleDarkMode = (useDarkMode, automatic) => {
    if (useDarkMode) {
        document.documentElement.setAttribute('data-bs-theme', 'dark')
        return;
    }

    if (automatic && window.matchMedia('(prefers-color-scheme: dark)').matches) {
        document.documentElement.setAttribute('data-bs-theme', 'dark')
    } else {
